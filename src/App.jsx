...
function App() {
  ...
  const processVideo = async (blob, filename) => {
    ...
    try {
        ...
        // RUMUS STABILISASI (-r 30, -pix_fmt yuv420p)
        ...
        setStatusDesc('File MP4 berhasil diunduh. Menutup tab dalam 3 detik...');
        
        // --- AUTO CLOSE ---
        setTimeout(() => {
            // Coba fokus balik (Best effort)
            try {
  ...
  const triggerDownload = (url, name) => {
      // Coba Web Share API dulu (Bagus untuk HP)
      if (navigator.canShare && navigator.share) {
          fetch(url).then(r => r.blob()).then(blob => {
            const file = new File([blob], name, { type: 'video/mp4' });
            if (navigator.canShare({ files: [file] })) {
                navigator.share({ files: [file], title: 'Video MP4' }).catch(() => {});
            } else {
                forceDownload(url, name);
            }
          }).catch(() => forceDownload(url, name));
      } else {
          forceDownload(url, name);
      }
  };
                ...
                <div style={styles.uploadBox}>
                    <div style={styles.warningBox}>
                        ...
                        <p style={{fontSize: '13px', color: '#555', lineHeight: '1.4'}}>
                            1. Kembali ke tab <b>Shortnews</b>.<br/>
